...
import WhiteArrowRight from '../../asset/images/white_>.png';
...
export class Chat extends Component<ChatProps, ChatState> {
  ...
  constructor(props: ChatProps) {
    super(props);
    this.chatSocket = new ChatSocketAPI({
  ...
  renderMsg(list) {
    ...
    const listItems = list.map((msg: MessageDto) => {
      const sender: UserDto | undefined = this.state.users.find((user) => {
      ...
      if (!msg.service) {
        if (
          this.state.user?.blockedUsers?.some((blocked) => {
            return blocked.id === sender?.id;
          })
        ) {
            ...
            return (
              ...
              >
                ...
                <div className="service_message">
                  messages(s) sent by blocked user(s)
                </div>
              </Stack>
            );
          }
      ...
      if (
        msg.service &&
        (msg.content === 'INVITE-EASY' ||
          msg.content === 'INVITE-MEDIUM' ||
          msg.content === 'INVITE-HARD') &&
      ...
      if (
        ...
        return (
          <Stack
            direction="column"
            spacing={1}
            className="invitation bor_green"
          >
            <Stack
              className="invitation_message"
              key={
                msg.date.toString() +
                sender?.color +
                sender?.login +
            ...
            >
              <SportsEsportsIcon
                className="invitation_icon green"
                style={{ width: '2vw', height: 'unset' }}
              />
            ...
            <Stack style={{ alignItems: 'center', paddingBottom: '0.5vw' }}>
              <div
                className="accept_invitation_button but_green"
                onClick={() => {
                  this.handleInvitation(msg.authorId, 0, msg.id);
                }}
              >
                Accept
              </div>
      ...
      if (
        ...
        return (
          <Stack
            direction="column"
            spacing={1}
            className="invitation bor_yellow"
          >
            <Stack
              className="invitation_message"
              key={
            ...
            >
              <SportsEsportsIcon
                className="invitation_icon yellow"
                style={{ width: '2vw', height: 'unset' }}
              />
              <div>{`${login} invite you to play`}</div>
            </Stack>
            ...
            <Stack style={{ alignItems: 'center', paddingBottom: '0.5vw' }}>
              <div
                className="accept_invitation_button but_green"
                onClick={() => {
                  this.handleInvitation(msg.authorId, 1, msg.id);
                }}
              >
                Accept
              </div>
          ...
          <Stack direction="column" spacing={1} className="invitation bor_red">
            ...
            <Stack style={{ alignItems: 'center', paddingBottom: '0.5vw' }}>
              <div
                className="accept_invitation_button but_green"
                onClick={() => {
                  this.handleInvitation(msg.authorId, 2, msg.id);
                }}
              >
                Accept
              </div>
      ...
      return (
        <div
          key={
            msg.date.toString() + sender?.color + sender?.login + sender?.avatar
          }
          className="message"
        >
          {' '}
  ...
  async onService(message: any) {
    ...
    if (message.content === 'PROFILED' || message.content === 'LOGINED') {
      const oldUser = this.state.users.find((user) => {
      ...
      const users: UserDto[] = this.state.users.filter((user) => {
        return user.id != message.authorId;
      });
      users.push(user);
      this.setState({
  ...
  componentWillUnmount() {
    if (this.chatSocket) this.chatSocket.cancel();
  }

  onKeyDown(e) {
    if (e.keyCode === 13) this.sendMessage(this.chanName);
  }
  ...
  async switchChannel(newChannelName: string) {
    ...
    if (this.props.isPrivateMessage) {
      const friend: UserDto | null = await UserAPI.getUserByLogin(
        newChannelName,
      );
      if (!friend) {
        this.setState({
      ...
      const chanId: number = await ChatAPI.createOrJoinPrivateMessage(
        friend.id,
      );
      channel = await ChatAPI.getChannelById(chanId, {
        withAdmin: true,
        withOwner: true,
      });
      if (!channel || !user) {
      ...
      channel = await ChatAPI.getChannelByName(this.chanName, {
        withAdmin: true,
        withOwner: true,
      });
      if (!channel || !user) {
  ...
  render() {
    ...
    return (
      <>
        <ol className="chat_list">{this.renderMsg(this.state.messages)}</ol>
        ...
        {this.state.isMuted && (
          ...
          >
            ...
            <div className="chat_bar_disabled">You are muted</div>

            <div className="invitation_send_msg_button but_grey">
              <img src={WhiteArrowRight} style={{ width: '75%' }} />
            </div>
          </Stack>
        )}
        ...
        {!this.state.isMuted && (
          ...
          >
            ...
            <div
              className="invitation_send_msg_button but_blue"
              onClick={() => {
