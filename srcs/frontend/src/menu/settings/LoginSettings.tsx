...
> {
  ...
  async updateLogin() {
    if (this.state.input === this.props.login)
      ...
      const isValid = await UserAPI.updateLogin(this.state.input.toLowerCase());
      if (isValid) {
  ...
  render() {
    return (
      <Grid
        ...
        sx={{ height: '33%' }}
      >
        <Grid item xs={4} className="grid_item_style bit9x9 white">
          NICKNAME
        </Grid>
        ...
        <Grid item xs={4} className="grid_item_style">
          {!this.state.editing && (
            <div
              ...
              onClick={() => {
                this.setState({ input: this.props.login || '', editing: true });
              }}
            >
              EDIT
            </div>
          ...
          {this.state.editing && (
            <div
              className="settings_button but_green"
              onClick={() => {
                this.updateLogin();
              }}
            >
