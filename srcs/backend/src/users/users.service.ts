...
export class UsersService {
  ...
  public async getOne(
    ...
    try {
      ...
      if (relationsPicker) {
        relationsPicker.withGames &&
          relations.push('games') &&
          relations.push('games.players');
  ...
  public async getOneBySocket(socketId: string): Promise<User | null> {
    try {
      ...
      users.forEach((user) => {
        if (
          user.socketIdTab &&
          user.socketIdTab[user.socketIdTab.indexOf(socketId)] === socketId
        )
          billy = user;
      });
      return billy;
  ...
  public async getChannels(userId: number): Promise<Channel[] | null> {
    ...
    if (user) return user.joinedChannels;
    return null;
  }
  ...
  public async setUserSocket(userId: number, socket: string): Promise<boolean> {
    const user: User | null = await this.getOne(userId);
    if (!user.socketIdTab) user.socketIdTab = [];
  ...
  public async addFriends(userId: number, friendsToAddIds: number[]) {
    ...
    friendsToAddIds.forEach(async (friendToAddId) => {
      if (user.friends.some((user) => user.id == friendToAddId)) return false;
      const friendToAdd: User = await this.getOne(friendToAddId);
      if (!friendToAdd || friendToAdd.id == userId) return false;
  ...
  public async updateLogin(userId: number, login: string): Promise<number> {
    const user: User | null = await this.getOne(userId, { withChannels: true });
    if (!user.firstLog) this.firstLogged(user);
    if (user.login == login) return 0;
    if (!(await this.login_available(login))) return 1;
    user.login = login;
    await this.userRepository.save(user);
    user.joinedChannels.forEach((channel) => {
  ...
  public async addBlockedUser(userId: number, blockedUser: number) {
    ...
    if (
      user.blockedUsers.some((user) => user.id == blockedUser) ||
      userId == blockedUser
    )
      return false;
  ...
  public async removeBlockedUser(userId: number, blockedUser: number) {
    ...
    if (
      ...
      return false;
    user.blockedUsers = user.blockedUsers.filter((user) => {
      return user.id != blockedUser;
    });
    await this.userRepository.save(user);
    return true;
  }
  ...
  public async userIsInChannel(
    userId: number,
    channelId: number,
  ): Promise<boolean> {
    const channels: Channel[] = await (
      await this.getOne(userId, { withChannels: true })
    ).joinedChannels;

    if (!channels) return false;
    const ret = channels.some((channel) => {
      return channel.id == channelId;
    });
    return ret;
  }

  public async userCanSendMessage(
    userId: number,
    channelId: number,
  ): Promise<boolean> {
    const channels: Channel[] = await (
