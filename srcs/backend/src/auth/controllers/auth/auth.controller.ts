...
export class AuthController {
  ...
  redirect(
    ...
    if (!request.user.firstLog) res.redirect(process.env.USER_INIT);
    else if (!request.user.twofa || session.istwofa)
      res.redirect(process.env.HOME_URL);
    else res.redirect(process.env.TWOFA_URL);
  }
    ...
    request.logOut((err) => {
      ...
      request.session.destroy((err) => {
        if (err) {
          return res
            .status(500)
            .send({ message: 'Session destruction failed' });
        }
